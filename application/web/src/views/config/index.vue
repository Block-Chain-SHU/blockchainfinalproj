    ...
    <div class="config-container">
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="12">
          <el-card class="config-card compact-card">
            <div slot="header" class="card-header">
              <i class="el-icon-s-operation" /> <span>追踪查询中节点标题</span>
            </div>
            <el-form label-width="90px" class="compact-form">
      ...
      <el-card class="config-card compact-card">
        <div slot="header" class="card-header">
          <i class="el-icon-edit-outline" /> <span>各角色培养信息配置</span>
          <el-radio-group v-model="z8" size="mini" class="role-tabs">
            <el-radio-button label="farmer">学生</el-radio-button>
            <el-radio-button label="factory">课程/实验室</el-radio-button>
            <el-radio-button label="driver">项目实践/竞赛导师</el-radio-button>
            <el-radio-button label="shop">就业单位/研究生去向</el-radio-button>
            <el-radio-button label="consumer">学院/企业/本人查询</el-radio-button>
          </el-radio-group>
        </div>
        <el-form label-width="140px" class="compact-form inline-form">
          ...
          <div v-else class="no-params-tip">
            <i class="el-icon-info" /> 查询角色无需配置参数标签
          </div>
        </el-form>
      </el-card>
  ...
  data() {
    return {
      ...
      z1: {
        ...
        parameters: {
          farmer: [
            { key: 'Fa_studentName', label: '学生姓名' },
            { key: 'Fa_major', label: '专业方向' },
            { key: 'Fa_enrollTime', label: '入学时间' },
            { key: 'Fa_graduationTime', label: '毕业时间' },
            { key: 'Fa_mentorName', label: '培养导师' }
          ],
          factory: [
            { key: 'Fac_courseOrLabName', label: '课程/实验室名称' },
            { key: 'Fac_courseTime', label: '开课/实验时间' },
            { key: 'Fac_platformName', label: '实验室/课程平台' },
            { key: 'Fac_courseBatch', label: '课程批次' },
            { key: 'Fac_contactNumber', label: '负责人联系方式' }
          ],
          driver: [
            { key: 'Dr_mentorName', label: '导师姓名' },
            { key: 'Dr_projectCode', label: '项目/竞赛编号' },
            { key: 'Dr_experienceYears', label: '指导年限' },
            { key: 'Dr_contribution', label: '项目贡献说明' },
            { key: 'Dr_phone', label: '联系电话' }
          ],
          shop: [
            { key: 'Sh_orgName', label: '单位/目标院校' },
            { key: 'Sh_orgAddress', label: '单位/院校地址' },
            { key: 'Sh_offerTime', label: '签约/录取时间' },
            { key: 'Sh_onboardTime', label: '入职/入学时间' },
            { key: 'Sh_contactPhone', label: '联系人方式' }
          ],
          consumer: []
        }
      },
